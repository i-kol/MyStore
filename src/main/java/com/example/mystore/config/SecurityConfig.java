...
import com.example.mystore.services.PersonDetailsService;
...
public class SecurityConfig{
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http
                ...
                .requestMatchers("/admin", "/product/add").hasRole("ADMIN") // указываем на то что страница /admin доступна пользователю с ролью ADMIN
...
//                .anyRequest().authenticated()
                .requestMatchers("/authentication", "/registration", "/error", "/resources/**", "/static/**", "/css/**", "/js/**", "/pics/**", "/images/**", "/product", "/product/info/{id}", "/product/search", "/product/searchHeader", "/logout").permitAll()
                ...
                .defaultSuccessUrl("/personalAccount", true) // Указываем на какой url необходимо направить пользователя после успешной аутентификации. Вторым аргументом указывается true чтобы перенаправление шло в любом случае послу успешной аутентификации
